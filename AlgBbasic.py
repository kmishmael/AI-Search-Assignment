...
def probability(i, tour, dist_matrix, num_cities, pheromone_levels):
    ...
    for j in range(num_cities): # j is the next possible city
        if j in tour or i == j: # ignores visited cities
            continue
...
def ant_colony_optimization(dist_matrix, num_cities, max_iterations, num_of_ants, initial_pheromone_level):
        ...
        while iteration < max_iterations:
            ...
            for a in range(num_of_ants): 
                ...
                while(len(candidate_tour) < num_cities):
                    probability_to_next_city = probability(city, candidate_tour, dist_matrix, num_cities, pheromone_levels)
                    largest_probability = max(probability_to_next_city)
                    # get the next city based on the probability 
                    next_city = list(probability_to_next_city).index(largest_probability)
            ...
            for row in range(len(pheromone_levels)):
                for col in range(len(pheromone_levels[row])):
                    ...
                    for a in range(num_of_ants):
                        ...
                        if tours[a].index(col) - tours[a].index(row) == 1:
                            overall_pheromone_change += (1/candidate_tour_distances[a])

                    pheromone_levels[row][col] = (1-decay_rate)*pheromone_levels[row][col] + overall_pheromone_change
...
tour_length = find_nearest_neighbour(dist_matrix, num_cities, 0)

num_of_ants = num_cities
# maximun number of iterations
max_iterations = 20
# calculate the initial pheromone level
initial_pheromone_level = num_of_ants / tour_length # initial pheromone level

tour, tour_length = ant_colony_optimization(dist_matrix, num_cities, max_iterations, num_of_ants, initial_pheromone_level)
