...
class Node:
    ...
    def __init__(self, state=[], cost=0, m=m) -> None:
        self.state = state
        self.cost = cost
        self.is_target_node = set(state) == cities
        self.child_cities = list(cities - set(state))
        self.f = m * self.get_heuristic_cost() + cost

    def get_heuristic_cost(self, n=n):
        ...
        if self.is_target_node:
            ...
            n = min(n, num_cities - len(state))

            if n == 1:
                return min([step_cost(state, state + [child]) for child in self.child_cities])

            else:
                pathways = list(permutations(self.child_cities, n))
                return min([sum(step_cost(state + list(path[:i]), state + list(path[:i+1])) for i in range(n)) for path in pathways])
...
def greedy_compete(current_node):
...
def a_star_search():
    ...
    while not p_queue.empty():
        ...
        if time.time() - start_time > limit:
            return greedy_compete(current_node)
        ...
        for child in current_node.child_cities:
            ...
            child_node = Node(
                ...
                cost=current_node.cost +
                step_cost(current_node.state, current_node.state + [child])
            )
