...
# create a set of numbers within range of the number of cities using set constructor
...
class Node:
    def __init__(self, state=[], cost=0):
        self.state = state
        self.cost = cost
        self.is_target_node = cities == set(state)
        self.child_cities = list(cities - set(state))
        self.f = self.get_heuristic_cost() + cost

    # returns distance between the Node and the target/final Node.
    def get_heuristic_cost(self):
        # get shallow copy
        state = self.state[:]

        # If Node is target, then the heuristic cost is 0.
        if self.is_target_node:
            return 0
        else:
            return min([step_cost(state, state + [child]) for child in self.child_cities])

    def __lt__(self, other):
        return self.f < other.f

# Calculate the step cost, "distance" from the root Node
def step_cost(current_state, child_state):
    
    visited_cities = len(current_state)

    if visited_cities == 0:
        return 0
    else:
        if visited_cities < num_cities - 1:
            return dist_matrix[current_state[-1]][child_state[-1]]
        else:
            # num_cities == visited_cities
            return dist_matrix[current_state[-1]][child_state[-1]] + dist_matrix[child_state[-1]][current_state[0]]

def greedy_coplete(current_node):
    state = current_node.state[:]
    cost = current_node.cost

    while len(state) != num_cities:
        mats = dist_matrix[state[-1]]
        # use math.inf from math libarary to set the minimum value
        min_mat = math.inf
        min_i = 0
...
def a_star():
    ...
    p_queue.put(initial_node)

    while not p_queue.empty():
        current_node = p_queue.get()
       # print('STATE = ',current_node.state,'SIZE = ', len(current_node.state),'COST = ', current_node.cost)
        if current_node.is_target_node:
            return current_node.state, current_node.cost

        if time.time() - start_time > limit:
            return greedy_coplete(current_node)

        for child in current_node.child_cities:

            child_node = Node(
                state = current_node.state + [child],
                cost = current_node.cost + step_cost(current_node.state, current_node.state + [child]),
            )
            
            p_queue.put(child_node)

start_time = time.time()

tour, tour_length = a_star()
